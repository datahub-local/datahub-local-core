    ...
    if [ -n "$CUSTOM_EXTRA_MODULES" ]; then
      ...
      pnpm add --shamefully-hoist --node-linker=hoisted --dangerously-allow-all-builds "$CUSTOM_EXTRA_MODULES" || echo "Error Installing extra modules: $CUSTOM_EXTRA_MODULES"
    fi
    ...
    if [ -n "$CUSTOM_COMMUNITY_NODES" ]; then
      ...
      su $NODE_USER -c "cd && pnpm install --shamefully-hoist --node-linker=hoisted --dangerously-allow-all-builds $CUSTOM_COMMUNITY_NODES" || echo "Error Installing community nodes: $CUSTOM_COMMUNITY_NODES"
    fi
